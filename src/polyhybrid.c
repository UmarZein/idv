...
void predict_ph(void *_state, float* out, const float* X, const float* Y, const bool* nans) {
...
void init_search_ph(void *_state){
    PolynomialHybrid *state = (PolynomialHybrid *)_state;
    fill0(state->weights);
    state->amp=1.3;
...
void search_eval_ph(void *_state){
    PolynomialHybrid *state = (PolynomialHybrid *)_state;
...
bool done_searching_ph(void *_state){
    PolynomialHybrid *state = (PolynomialHybrid *)_state;
    return state->amp<0.01;
}
void search_step_begin_ph(void *_state){
    PolynomialHybrid *state = (PolynomialHybrid *)_state;
    for (int i=0; i<W; i++){
        state->weights[i] = rand_normal()*state->amp+state->best_weights[i];
    }
}
void search_step_end_ph(void *_state, const float total_error){
    PolynomialHybrid *state = (PolynomialHybrid *)_state;
...
void finish_search_ph(void *_state){
    PolynomialHybrid *state = (PolynomialHybrid *)_state;
    for (int i=0; i<W; i++){
...
PolynomialHybrid create_polyhybrid(const Flags flags){
    ...
    model.base.predict = predict_ph;
    model.base.init_search = init_search_ph;
    model.base.search_eval = search_eval_ph;
    model.base.search_step_begin = search_step_begin_ph;
    model.base.search_step_end = search_step_end_ph;
    model.base.finish_search = finish_search_ph;
    model.base.done_searching = done_searching_ph;
    return model;
