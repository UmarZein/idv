// copy of 2code.c, except we use elastic ewma
#include <math.h>
#include <stdio.h>
#include <stdlib.h>
#include <time.h>
...
struct Output inner(int argc, char *argv[]) {
    ...
    long long timestamp;
    long long last_ewma_timestamp = -9999999;
    float target;
    ...
    while (fscanf_s(file, "%lld,%f", &timestamp, &target) == 2) {
        ...
        ffillf(ewms);
        ewms[W-1]=ewma2(values[W-1],ewms[W-2],decay_rate, (float)(timestamps[W-1]-last_ewma_timestamp)/SECOND_SCALE/(60.0*10.0));
