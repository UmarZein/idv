//u2code.c except extrapolation just uses the nearest known value
#include "./utils.c"
            ...
            if (cur_variabilty>0.5){
                for (int i=0; i<W; i++){
                    if (!hit_leftmost){
                        ...
                        if (!nansOrAnomalies[i]) carried_number=truths[i];
                        ewmas[i]=DECAY_RATE*ewmas[i-1]+(1.0-DECAY_RATE)*carried_number;
                    }
                }
            }
            ...
            for (int i=0; i<W; i++){
                if (nansOrAnomalies[i]){
                    ...
                    if (i<L) prediction=leftmost;
                        ...
                        prediction += weights[deg]*interpLagrange(timestamps, ewmas, closestMask, (float)i);
                    }
                    const float error = prediction-ewmas[i];
