...
struct Output inner(int argc, char *argv[]) {
    ...
    while (fscanf_s(file, "%lld,%f", &timestamp, &target) == 2) {
        ...
        cur_median = median(values, nanOrAnomaly);// unused
        cur_mad = mad(values, nanOrAnomaly);// unused
        float rightmost_non_anomaly = get_rightmost_number(values, anomalies);
        if (!is_nan(rightmost_non_anomaly))
            ewms[W-1]=ewma2(rightmost_non_anomaly,ewms[W-2], decay_rate, (float)(timestamp-last_ewma_timestamp)/SECOND_SCALE/60.0/10.0);
        else//if somehow every element in the window is an anomaly
            ewms[W-1]=ewma2(median(values, nans),ewms[W-2], decay_rate, (float)(timestamp-last_ewma_timestamp)/SECOND_SCALE/60.0/10.0);
